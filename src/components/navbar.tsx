  ...
  return (
    <div className={`${styles.flexBetween} w-full  navbar`}>
      ...
      <div className="sm:hidden flex flex-1 justify-end items-end">
        ...
        >
          <ul className={`list-none ${styles.mmStyle} xs:flex justify-center items-center flex-1 `}>
        {navigationLinks.map((item) => (
          <li key={item.id} className={`${styles.mmList}`}>
            <a
              href={`#${item.id}`}
              className={`${styles.navLink} mr-10 ${active === item.id ? "active" : ""}
               hover:text-white transition-all duration-300 navlink 
               `}
              onClick={() => activeHandler(item.id)}
            >
